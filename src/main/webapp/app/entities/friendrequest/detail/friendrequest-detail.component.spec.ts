...
describe('Friendrequest Management Detail Component', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [FriendrequestDetailComponent],
      providers: [
        provideRouter(
          [
            {
              ...
              resolve: { friendrequest: () => of({ id: 'ABC' }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load friendrequest on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', FriendrequestDetailComponent);

      // THEN
      expect(instance.friendrequest()).toEqual(expect.objectContaining({ id: 'ABC' }));
    });
  });
