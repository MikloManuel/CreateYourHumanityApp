...
public class KeyTableGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all keyTables").get("/api/key-tables").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new keyTable")
                        .post("/api/key-tables")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_keyTable_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created keyTable").get("${new_keyTable_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created keyTable").delete("${new_keyTable_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the KeyTable entity").exec(scn);
