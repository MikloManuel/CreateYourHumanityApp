...
public class MindmapXmlGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all mindmapXmls").get("/api/mindmap-xmls").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mindmapXml")
                        .post("/api/mindmap-xmls")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"text\": \"SAMPLE_TEXT\"" + ", \"modified\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mindmapXml_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mindmapXml").get("${new_mindmapXml_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mindmapXml").delete("${new_mindmapXml_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MindmapXml entity").exec(scn);
