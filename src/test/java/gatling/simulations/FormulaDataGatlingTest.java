...
public class FormulaDataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all formulaData").get("/api/formula-data").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formulaData")
                        .post("/api/formula-data")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formulaData_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created formulaData").get("${new_formulaData_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created formulaData").delete("${new_formulaData_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormulaData entity").exec(scn);
