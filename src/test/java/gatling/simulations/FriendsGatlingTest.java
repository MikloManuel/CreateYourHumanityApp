 ...
 */
public class FriendsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all friends").get("/api/friends").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new friends")
                        .post("/api/friends")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"connectDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"friendId\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_friends_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created friends").get("${new_friends_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created friends").delete("${new_friends_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Friends entity").exec(scn);
