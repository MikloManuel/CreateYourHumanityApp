...
public class GrantSettingsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all grantSettings").get("/api/grant-settings").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new grantSettings")
                        .post("/api/grant-settings")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"grantMap\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_grantSettings_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created grantSettings").get("${new_grantSettings_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created grantSettings").delete("${new_grantSettings_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GrantSettings entity").exec(scn);
