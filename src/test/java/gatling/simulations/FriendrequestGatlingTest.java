 ...
 */
public class FriendrequestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all friendrequests").get("/api/friendrequests").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new friendrequest")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_friendrequest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created friendrequest").get("${new_friendrequest_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created friendrequest").delete("${new_friendrequest_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Friendrequest entity").exec(scn);
