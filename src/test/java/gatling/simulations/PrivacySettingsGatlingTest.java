...
public class PrivacySettingsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all privacySettings").get("/api/privacy-settings").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new privacySettings")
                        .post("/api/privacy-settings")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"settingsMap\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_privacySettings_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created privacySettings").get("${new_privacySettings_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created privacySettings").delete("${new_privacySettings_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PrivacySettings entity").exec(scn);
